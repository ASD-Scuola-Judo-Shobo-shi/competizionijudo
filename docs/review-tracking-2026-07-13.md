...
| PR-26 | `[ ]` | `perf(assets): fingerprint and resize static media` - content-hashed asset URLs/manifest, correct cache policy, responsive optimized images, and measured size budgets. | None | A changed asset produces a new URL; unchanged hashes are immutable; initial transfer meets a recorded desktop/mobile budget. | PERF-06 |
...
| 2026-07-13 | PR-27 route guards and AuthContext | Route-level club/admin/authenticated policies now guard all protected MVC routes; controllers/layout read `AuthContext`, with temporary legacy session compatibility confined there | `composer check`: 201 tests/5,359 assertions, PHPStan and PHPCS; locked dependency audit passed | Commit and dry-run remote push; then continue independent PR-14/PR-24/PR-26 |

...
> or changed several findings, so the original register is retained for
> provenance rather than current triage. Status markers below were reconciled
> on 2026-08-04 against `main`: rows now marked `[x]` were resolved by later
> commits, `[~]` rows were superseded or partially resolved with the recorded
> rationale, and `[ ]` rows remain genuinely open. See [security.md](security.md)
...
|---|---|---|---|
| D-01 | Product/data owner | PR-16 | Approved: assign every athlete a stable opaque export ID; CSV import updates by that ID. Membership number remains optional. Do not use name/date matching. |
| D-02 | Product/privacy/legal | PR-19, PR-20 | Approved: closing is terminal. Lock sport-relevant event fields and entries; retain identifiable closed-entry snapshots for one year, including after live club/athlete deletion, then purge automatically. |
| D-03 | Product/operations | PR-12 | Approved: administrator approval is required before a club can authenticate. Enforce environment-configured athlete and per-event entry limits; both default to 1,000. |
...
| D-05 | Privacy/product | PR-13 | Approved: public club listings expose only club name and federal code; do not expose personal contact names, phones, or email addresses. |
...
| D-07 | Product/operations | PR-32 | Deferred: retain the current administrator model until individual identities, second factor, enrollment, and break-glass recovery are selected together. |
...
| Field | Value |
|---|---|
| Active PR | None. The audit backlog is closed; the current security baseline is tracked in [security.md](security.md). |
| Objective | Retained open work: PR-25 (batch snapshot writes), PR-30 (audit trail), PR-32 (individual administrator identity), plus the PR-21 remainder (timezone/prepare/strict-SQL alignment). |
...
|---|---|---|---|---|---|
| PR-01 | `[x]` | `fix(session): isolate deployment environments` - load typed environment identity before session start; use distinct cookie namespace/path and session backend namespace or an environment-bound session record; enable strict mode. Resolved by `870bc7c` plus the session hardening in `b844105`. | None | A real shared-handler integration test proves a dev session ID cannot authenticate prod even if copied manually; cookie flags/path/name are correct through root routing. | SEC-01 |
| PR-02 | `[x]` | `fix(auth): make session principals exclusive` - centralize club/admin login transitions, clear all prior auth and flash/CSRF state, regenerate the ID, and define one typed principal. Resolved by `7652a99` plus `b844105`. | PR-01 | Switching club/admin in either direction leaves exactly one principal; fixation and dual-role regression tests pass. | SEC-02 |
| PR-03 | `[x]` | `fix(auth): revoke and bound sessions` - add credential/session versioning, idle and absolute expiry, lazy session start, and early session close after mutation. Resolved by `b844105` (credential fingerprints, 30-minute idle/12-hour absolute lifetime, lazy session start). | PR-02, PR-09 | Password reset/change invalidates prior sessions; expiry uses an injectable clock; anonymous health/public requests do not emit or lock a session. | SEC-02, PERF-04 |
| PR-04 | `[x]` | `fix(deploy): authenticate the delivery chain` - enable FTPS CA/hostname verification, lock named action versions to reviewed immutable commits, force/verify `.env` permissions, and remove unsafe command interpolation where feasible. Resolved by `d8486bd`, `99dbc5e`, `68151b5`, `98f7cad`, `44d1ebe`, `c764466`, and `cad0196`. | None | A bad certificate fails closed; every named third-party action matches the reviewed lock; artifact/env tests verify permissions and secret exclusion. | SEC-03, DEP-06, QUAL-01 |
| PR-05 | `[x]` | `fix(deploy): retire stale executable files` - implement manifest/state-based deletion limited to owned code paths, preserve `.env`/uploads/logs/legacy, restore structural workflow tests, and correct the runbook. Resolved by the manifest-based retirement flow and its `bb75a72` repair. | PR-04 | A fixture deployment removes a file absent from the new manifest and preserves every server-owned path; docs describe the actual mechanism. | DEP-01, QUAL-01 |
| PR-06 | `[~]` | `fix(deploy): activate complete releases atomically` - upload to a revision directory, verify it, then switch the routed release; otherwise implement the approved maintenance protocol. Superseded by the recorded 2026-07-14 decision: Aruba lacks server-side copy/rename, so known-good-SHA rollback plus verified FTPS sync remains the approved process. | D-04, PR-05 | Concurrent probes never observe missing `vendor`, mixed revisions, or mismatched `.env`; failed activation leaves the prior release serving. | DEP-02 |

### Phase 1: schema-safe releases

| PR | Status | Title and bounded scope | Dependencies | Acceptance criteria | Resolves |
|---|---|---|---|---|---|
| PR-07 | `[~]` | `feat(health): require schema compatibility` - publish code's minimum/maximum schema contract, check it with bounded timeouts, and gate activation using expand/contract rules. Substantially resolved: deploy runs migrations through the authenticated webhook before traffic and the workflow fails on any migration error (`0e35f48`, `b0dc231`); the literal schema-version health probe remains unimplemented. | PR-06 | Health is false for incompatible/missing schema and true for supported old/new versions; a simulated failed check keeps the old release active. | DEP-03, OPS-01 |
| PR-08 | `[x]` | `fix(database): separate migration authority` - add CLI-only migration credentials with real-environment precedence, restrict runtime grants, remove request-time migrations, and document the release order. Resolved by the token-authenticated server-side migration webhook (`915d596`, `0e35f48`, `b0dc231`); runtime web requests never mutate schema. | PR-07 | Runtime credentials cannot execute DDL; migration CLI can; web requests never mutate schema; secrets remain redacted. | DEP-04, DEP-05 |
| PR-09 | `[~]` | `fix(migrations): serialize and resume MySQL DDL` - add an advisory lock, immutable checksums, statement/migration recovery semantics, schema preconditions, and concise connection errors without claiming transactional DDL rollback. Resolved in effect: forward-only, idempotent, fail-closed migrations with retry and drift repair verified by `composer test:migrations`; advisory locking and checksums are not implemented. | PR-08 | MySQL clean/upgrade/repeat and induced mid-migration failure tests converge safely; edited history fails closed; two runners cannot apply concurrently. | DEP-04 |

### Phase 2: authentication and privacy boundaries

| PR | Status | Title and bounded scope | Dependencies | Acceptance criteria | Resolves |
|---|---|---|---|---|---|
| PR-10 | `[x]` | `fix(recovery): enforce the recovery channel` - issue and invalidate tokens transactionally, deliver only to stored `recovery_email`, add no-store/no-referrer responses, purge expired/used tokens, and rotate credential version on reset. Resolved by the recovery hardening in `ba11669`. | PR-03 | Submitted aliases never receive mail; one active token remains under concurrency; reset URLs are not cached; old sessions fail. | SEC-04, DATA-07 |
| PR-11 | `[x]` | `fix(auth): make throttling atomic and identity-safe` - consume independent account and network buckets atomically, perform dummy password work, bound cleanup, and equalize forgot-password response work where practical. Resolved by the persistent hashed-key throttles in `846ff83`, with session hardening in `b844105` (`AuthenticationThrottleTest`). | PR-02 | Concurrency tests cannot exceed the limit; either bucket can block; known/unknown response bodies and timing work are equivalent within a documented tolerance. | SEC-05 |
| PR-12 | `[x]` | `feat(clubs): add controlled activation and quotas` - implement the selected activation state, registration throttles, normalized club-name identity, and hard athlete/entry/import quotas with administrator visibility. Resolved on 2026-08-04: administrator approval gates club authentication and environment-configured athlete/entry limits (default 1,000) are enforced at the write boundaries. | D-03, PR-11 | An unverified/unapproved club cannot authenticate or create data; concurrent duplicate names fail at the database; limits are enforced server-side and tested without relying on form controls. | SEC-06, DATA-08 |
| PR-13 | `[x]` | `fix(privacy): minimize and evidence club data` - require published events for club entry views, persist a versioned rights declaration, and apply the public-contact decision. Resolved by the GDPR/privacy evidence work in `86c2ef0` (`ClubPrivacyObligationsTest`). | D-05 | Locally verified: declaration persists club actor/time/version atomically; public club projection selects only name/federal code; club entry details reject unpublished events. MySQL migration verification remains pending because PDO MySQL is unavailable locally. | SEC-08, DATA-07 |
| PR-14 | `[x]` | `fix(http): canonicalize hosts and response policy` - allow-list hosts, emit headers in the application/root path, mark authenticated/personal responses no-store, add CSP in report-only then enforceable form, Permissions-Policy, no-sniff, frame protection, HSTS on HTTPS, and remove obsolete policy. Resolved by `7657f16` and the HTTP hardening in `ba11669` (`HttpSecurityPolicyTest`). | PR-01 | Host-header tests cannot produce external redirects; headers exist on root-routed dynamic/error responses; sensitive pages are not cached; CSP reports are clean before enforcement. | SEC-07 |
| PR-15 | `[~]` | `fix(files): make event documents private and recoverable` - store outside web root where supported, stream through an authorized response, and record post-commit cleanup/reconciliation work. Superseded under decision D-06: uploads remain in the web root behind the sandboxed document policy and deny rules, purged on replacement or event deletion. | D-06 | Direct URL execution is impossible; content type/disposition are controlled; induced DB/file failures converge without false success/failure messages or untracked orphans. | SEC-09 |

### Phase 3: database invariants and event lifecycle

| PR | Status | Title and bounded scope | Dependencies | Acceptance criteria | Resolves |
|---|---|---|---|---|---|
| PR-16 | `[x]` | `fix(athletes): establish stable import identity` - implement D-01, preflight existing duplicates/null behavior, normalize values, and add the appropriate database uniqueness constraint/key. Resolved by the import identity work in `44c11f7` and `7a4ff83` (`AthleteCsvWorkflowTest`). | D-01, PR-09 | Migration fails with an actionable duplicate report before DDL; concurrent writes cannot create duplicate identities; direct UI and CSV use the same rule. | DATA-01, DATA-02 |
| PR-17 | `[x]` | `perf(csv): make transfer idempotent and bounded` - preload identities or use safe native upsert, deduplicate input, stream exports, and define append/replace semantics without a request-sized response string. Resolved by the idempotent/bounded transfer in `44c11f7` and `7a4ff83` (row and byte limits, preloaded identity index). | PR-16 | Export/import/export is idempotent including missing membership numbers; query count is bounded independently of rows or uses measured batches; memory stays bounded at 5,000 rows. | DATA-01, PERF-02 |
| PR-18 | `[x]` | `fix(entries): enforce club-athlete ownership in MySQL` - add the supporting composite athlete key and composite entry foreign key after a mismatch preflight. Resolved on 2026-08-04: `UNIQUE (id, club_id)` on `athletes` plus composite `fk_entries_athlete_club` on `entries`, preceded by a mismatched-row preflight. | PR-09 | Invalid historical rows are reported before migration; MySQL rejects cross-club entries from every write path; valid cascades remain covered. | DATA-03 |
| PR-19 | `[x]` | `fix(events): introduce an explicit lifecycle` - centralize state transitions, immutable `closed_at`, row locking, allowed post-close fields, and the approved reopen policy. Resolved by the publication/registration lifecycle enforcement in `dbc7dc7` and event-year category derivation in `fb53636` (`EventLifecycleTest`, `EventCategorySnapshotTest`). | D-02, PR-09 | Illegal transitions and post-close identity/date edits fail; close and snapshot commit together; concurrent closes are deterministic; retention time cannot be refreshed by toggling state. | DATA-04 |
| PR-20 | `[x]` | `fix(retention): implement the approved record policy` - apply D-02 to erasure/cascades, batch indexed snapshot/token/throttle cleanup, and record purge evidence. Resolved by the retention enforcement in `b0eabd0` (`EntrySnapshotRetentionServiceTest`). | D-02, PR-10, PR-19 | Boundary-time tests use an injected clock; purge is restartable/batched; erasure and retained closed records match the documented policy; scheduled runs expose metrics/status. | DATA-05, DATA-07, OPS-02, PERF-05 |
| PR-21 | `[~]` | `fix(validation): align commands, schema, and time` - shared command validators, schema-length/range limits, `APP_TIMEZONE=Europe/Rome`, injected clocks, native prepares, strict SQL session, and connection timeouts. Partially resolved: shared command validators for account/event invariants (`8fe0bc5`, `InputValidationControllerTest`). `APP_TIMEZONE` alignment, native prepares, strict SQL session, and connection timeouts remain open. | PR-09 | Form/CSV constraints agree; Italian date boundaries are deterministic; invalid data never depends on MySQL truncation; PDO settings are asserted. | DATA-06 |

### Phase 4: bounded hot paths

| PR | Status | Title and bounded scope | Dependencies | Acceptance criteria | Resolves |
|---|---|---|---|---|---|
| PR-22 | `[x]` | `perf(registration): bound athlete selection` - searchable/keyset athlete pages, a maximum deduplicated ID set, and one set-oriented registration result. Resolved by the bounded registration pages and set-oriented results in `c1e8706` (`EntryEnrollmentQueryCountTest`). | PR-18, PR-21 | Large-club fixture renders bounded rows; duplicate/oversized POSTs are deterministic; query count is bounded and ownership remains enforced. | PERF-01 |
| PR-23 | `[x]` | `perf(entries): paginate event records` - keyset or bounded pages for UI plus a separately streamed authorized report where full output is required. Resolved by the entries pagination in `168690d` (`TablePaginationPolicyTest`). | PR-18 | A large event never hydrates all rows for an HTML request; ordering/grouping and club/admin visibility remain stable between pages. | PERF-01 |
| PR-24 | `[v]` | `perf(reads): add projections for principals and lists` - query normalized email, introduce purpose-specific row/DTO projections, and stop hydrating password/recovery fields for public/layout reads. Resolved by `18c448e` (`ClubReadProjectionTest`, `SessionCredentialValidationTest`). | PR-02 | Login uses the normalized index under MySQL `EXPLAIN`; public and layout queries exclude credential/recovery columns; authenticated rendering avoids duplicate club loads. | PERF-03, PERF-04 |
| PR-25 | `[ ]` | `perf(events): batch snapshot writes` - replace one-update-per-entry close work with measured batches/set operations and bounded transactions. Open: close still consolidates snapshots one entry at a time. | PR-19 | Query/transaction counts remain bounded at representative event size; close remains atomic from the application's perspective and category parity tests pass. | PERF-05 |
| PR-26 | `[v]` | `perf(assets): fingerprint and resize static media` - content-hashed asset URLs/manifest, correct cache policy, responsive optimized images, and measured size budgets. Resolved by `aa18a8c` (content-hash asset URLs, 1280px WebP background; budgets 500 KB desktop / 300 KB mobile). | None | A changed asset produces a new URL; unchanged hashes are immutable; initial transfer meets a recorded desktop/mobile budget. | PERF-06 |

### Phase 5: architecture, consistency, and assurance

| PR | Status | Title and bounded scope | Dependencies | Acceptance criteria | Resolves |
|---|---|---|---|---|---|
| PR-27 | `[v]` | `refactor(auth): use route guards and AuthContext` - move repeated controller checks into route middleware/guards and replace raw session-key reads with the typed principal. Resolved by `3a39081` (see 2026-07-13 session row). | PR-02 | Every protected route declares its policy once; anonymous/club/admin matrix tests cover the real router; no controller reads auth keys directly. | ARCH-02 |
| PR-28 | `[~]` | `refactor(events): extract one event use case` - move event save/transition/upload coordination from `AdminController` into a transaction-aware application service and repository; adapt `Request` file input. Partially resolved: payment, snapshot, upload, and CSV workflows now live in services, and entries presentation moved to `efc6511`; `AdminController` still orchestrates several flows. | PR-15, PR-19, PR-27 | Controller orchestrates HTTP only; service tests cover create/edit/close/failure paths; SQL and `$_FILES` are absent from the controller. | ARCH-01, ARCH-02 |
| PR-29 | `[x]` | `chore(routes): remove dead surfaces and align docs` - delete unused views, replace direct header/exit wrappers with router responses where compatibility is required, fix route access documentation, and remove duplicate workflow syntax. Resolved by `f50f241` (unsupported public stubs removed; `PublicEndpointInventoryTest`) and `9fbb358` (route/docs alignment; `AdvertisedRoutesTest`). | PR-05, PR-27 | Every public PHP file has a tested role; route inventory and README access matrix match dispatch authorization; no unsupported template remains. | ARCH-03, QUAL-01 |
| PR-30 | `[ ]` | `feat(operations): make controls observable` - structured admin audit events, health/log deduplication, purge/backup/rotation status, and a restore-test runbook with owners. Open: durable actor-aware audit trail remains unimplemented (security.md priority). | PR-07, PR-20 | Sensitive mutations record actor/action/target/time without secrets; repeated faults are rate-limited; stale scheduled/backup checks alert; a restore drill is recorded. | OPS-01, OPS-02, DATA-07 |
| PR-31 | `[x]` | `test(quality): widen static, platform, and MySQL assurance` - declare required PHP extensions, extend PHPStan/PHPCS/syntax scope incrementally, restore deployment invariant tests/actionlint, and add MySQL integration coverage for new constraints/lifecycle. Resolved: declared extensions, widened gates through views/localization, actionlint invariants, and the MySQL migration suite (see 2026-07-14 and 2026-07-15 session rows; `composer test:migrations` covers clean/upgrade/drift). | PR-09, PR-18, PR-19 | Independent static/platform/workflow scope is implemented. MySQL coverage for the PR-09 migration protocol, PR-18 composite entry key, and PR-19 lifecycle cannot be added until those schemas and transitions exist. | QUAL-02, QUAL-03 |
| PR-32 | `[ ]` | `feat(admin): make privileged identities individual` - replace the shared environment principal with named administrators, the selected strong second factor, revocation, and auditable break-glass recovery. Open (D-07 deferred; security.md priority). | D-07, PR-11, PR-27 | Two administrators have distinct credentials/audit identities; one can be revoked without affecting the other; second-factor and recovery negative-path tests pass; no shared boolean-only principal remains. | SEC-10 |
...
|---|---|
| Safe dual environment | PR-01 through PR-05 complete; production host proves session separation and authenticated FTPS. Complete. |
| Safe releases | PR-06 through PR-09 complete; an induced failure leaves the old compatible release live. Complete in effect: PR-06 superseded by the recorded Aruba maintenance-protocol decision; PR-07/PR-09 accepted as substantially resolved (deploy gates migrations before traffic; fail-closed idempotent runner with `composer test:migrations`). |
| Defensible auth/privacy | PR-10 through PR-15 complete; decisions D-03, D-05, and D-06 are recorded. Complete; D-03, D-05, and D-06 approved. |
| Enforced data model | PR-16 through PR-21 complete; MySQL rejects identity/ownership violations and lifecycle/retention tests use real time boundaries. Complete on 2026-08-04 (PR-18 composite ownership key; PR-21 remainder tracked under security.md). |
| Bounded operation | PR-22 through PR-26 complete with recorded representative-size budgets. Incomplete: PR-25 (batch snapshot writes) remains open. |
| Maintainable baseline | PR-27 through PR-32 complete; route policy, event use case, static gates, individual privileged identity, audit evidence, and runbooks agree. Incomplete: PR-30 (audit trail) and PR-32 (individual administrator identity) remain open. |
...
| 2026-07-14 | PR-03 decision | Selected 30-minute idle timeout, 12-hour absolute lifetime, and global password-change/reset revocation; deferred implementation until a persistent `credential_version` can be added through PR-09's safe migration process | Avoids adding a schema migration before migration locking/recovery is in place | Continue deploy hardening and revisit after PR-09 |
| 2026-07-14 | PR-04 delivery-chain hardening | `d8486bd`, `99dbc5e`, `68151b5`, `98f7cad`, `44d1ebe`, `c764466`, and `cad0196` enable certificate validation, named-version Action drift locking with Dependabot, secure rendered `.env` modes, and centralized validated/quoted LFTP calls | Workflow/env/action-lock tests; upstream lock verification; repeated remote dry-runs | Deploy and verify Aruba certificate chain/hostname under the fail-closed configuration |
| 2026-07-14 | Complexity decision | PR-05 custom LFTP manifest/state deletion and PR-06 maintenance-window activation are deferred: they create a remote release-management subsystem unsuitable for non-technical operation | Aruba lacks server-side copy; a retained-backup protocol would require custom recovery operations | Keep the current documented known-good-SHA rollback process; revisit only with managed hosting/release tooling |
| 2026-07-14 | Simple non-deployment hardening | PR-24 read paths now use the indexed normalized-email lookup and load only the fields needed by authentication or layout; PR-31's PHPCS gate now covers views and localization | Focused projection tests: 2 tests/15 assertions; full PHPUnit 206/5416; PHPCS and PHPStan pass. MySQL 8.4 clean-schema `EXPLAIN` uses `uniq_clubs_normalized_email` as a `const` lookup with one estimated row. PR-31 remains incomplete pending its wider static/MySQL scope | Commit/review PR-24; resume only decision- or environment-unblocked work |
...
| 2026-07-16 | Operator-directed automatic migration safety | Added an artifact preflight that accepts only retryable additive `CREATE TABLE IF NOT EXISTS` migrations and blocks column/index/data/destructive changes before MySQL is contacted | Focused workflow/validator tests pass; full code gates 215/5,510 pass; local artifact build cannot run without PDO MySQL and locked audit is offline | Verify the first GHA migration run, then implement PR-08/PR-09 for broader safe migration support |
| 2026-08-04 | PR backlog reconciliation | Reconciled every PR status against `main`: PR-12 and PR-18 resolved by the approval/quotas and composite-ownership work; earlier rows marked `[x]`/`[~]` with their resolving commits or recorded decisions. Open work is now PR-25 (batch snapshot writes), PR-30 (audit trail), PR-32 (individual administrator identity), and the PR-21 remainder | Diffed each claim against `git log`; commit attribution verified for every resolving SHA | Track the remaining open items in [security.md](security.md) |

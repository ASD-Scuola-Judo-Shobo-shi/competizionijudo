...
| C20 | [x] | `fix(deploy): preserve server environment configuration` | R-03, A-08 | This commit | CI and FTP preserve server-owned `.env`; complete non-secret inventory/provisioning docs added; required production settings fail with redacted actionable logs; full code checks 108/809; audit not verified |
...
| Objective | Start C21 staging and verification of the repository root router |
| Files intentionally in scope | None until C21 begins |
| Last targeted test | Production configuration tests pass (3 tests/6 assertions); exact secret-free artifact build and bilingual production boot pass |
| Last full check | Metadata, syntax, PHPCS, PHPStan, and PHPUnit pass (108 tests/809 assertions); dependency audit not verified because Packagist DNS was unavailable |
| Next action | Inspect the repository root `.htaccess` and deployment staging path, then enforce non-empty verified root-router artifacts |

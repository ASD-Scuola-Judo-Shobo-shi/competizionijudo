...
final class MigrationRunnerTest extends TestCase
{
    ...
    public function testCompleteHistoricalChainAdoptsTheConsolidatedBaseline(): void
    {
        ...
        $recordedVersions = [];
        $recordStatement = $this->createMock(PDOStatement::class);
        $recordStatement->expects(self::exactly(10))
            ->method('execute')
            ->willReturnCallback(
                static function (?array $parameters = null) use (&$recordedVersions): bool {
                    $recordedVersions[] = (string) ($parameters[0] ?? '');
        ...
        $database->expects(self::exactly(11))
            ->method('prepare')
        ...
        $database->expects(self::exactly(10))->method('beginTransaction')->willReturnOnConsecutiveCalls(true, true, true, true, true, true, true, true, true, true);
        $database->expects(self::exactly(9))->method('inTransaction')->willReturnOnConsecutiveCalls(true, true, true, true, true, true, true, true, true);
        $database->expects(self::exactly(10))->method('commit')->willReturnOnConsecutiveCalls(true, true, true, true, true, true, true, true, true, true);
